<template>
  <section id="projects" class="projects">
    <div class="proj-inner">
      ...
      <!-- GitHub trophy banner -->
      <div class="gh-trophy">
        <div class="trophy-item">
          <GitFork :size="15" />
          <span>{{ ghStats.repos }} {{ t('hero.stats.repos') }}</span>
        </div>
        <div class="trophy-item">
          <Star :size="15" />
          <span>{{ fmt(ghStats.stars) }} {{ t('hero.stats.stars') }}</span>
        </div>
      </div>
...
import { Github, Star, GitFork, ExternalLink, CircleDot, Tag } from 'lucide-vue-next';
...
const ghStats = computed(() => stats.value?.user ?? { repos: 39, stars: 280, forks: 25 });
...
.trophy-item.trophy-badge { color: var(--accent-warm); border-color: color-mix(in srgb, var(--accent-warm) 30%, transparent); }
...
.trophy-card svg { color: var(--accent-warm); }
